...
```{r}
ggplot(tabela2) +
  geom_boxplot(aes(x = Namen, y = `Velikost kredita`, fill = Ocena), position = "dodge") +
  theme(text = element_text(size = 9)) + scale_x_discrete(guide = guide_axis(n.dodge = 2))


tabela2 %>% group_by(Namen, Ocena) %>% summarise(`Število` = n()) %>% kable()
```
